...
class StreamNotifier extends StateNotifier<StreamState> {
  ...
  StreamNotifier(this._camera, this._ws)
      : super(const StreamState(streaming: false));
  ...
  Future<void> startStreaming(String incidentId) async {
    ...
    _chunkTimer = Timer.periodic(
        const Duration(milliseconds: AppConstants.mediaChunkIntervalMs), (_) {
  ...
  Future<void> _captureAndSend(String incidentId) async {
    ...
    try {
      ...
      try {
        videoFrame = await _camera.captureFrame();
      } catch (_) {
        videoFrame = '';
      }

      try {
        audioChunk = await _camera.captureAudio();
      } catch (_) {
        audioChunk = '';
      }
